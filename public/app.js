document.addEventListener("DOMContentLoaded", () => {
        ...
        try {
            ...
            if (isAutoRefresh) {
                ...
                if (newNotams.length > 0) {
                    ...
                    showNewNotamAlert(`${icao}: ${newNotams.length} new NOTAM${newNotams.length > 1 ? 's' : ''} detected!`, icao, newNotams[0].number);
                }
            }
    ...
    // All other UI functions (renderTabs, renderCards, notamCardHtml, etc.) should be pasted here.
    // To avoid excessive length, I'll define a few key ones and assume the rest are the same.
